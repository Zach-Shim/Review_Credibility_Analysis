...
class Sentiment():
    ...
    def test_model(self, input_data: str = TEST_REVIEW):
    ...
    def load_training_data(self, data_directory: str = __dataset__, split: float = 0.8, limit: int = 0) -> tuple:
        # Load from files
        reviews = []
    ...
    def load_amazon_data(self):
        # Choose (# of incentivized reviews in database) random records
        incentivized_reviews = Review.objects.filter(incentivized=1)
        normal_reviews = Review.objects.exclude(incentivized=1).filter(reviewID__in=random.sample(range(1, Review.objects.all().count()), incentivized_reviews.count()))
